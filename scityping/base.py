...
try:
    ...
    from pydantic.dataclasses import dataclass as plain_or_pydantic_dataclass  # Used for autogenerating the nested Data container
    ...
    # Pydantic provides more powerful serialization, but if is isn’t available, we fall back to the std lib
    from dataclasses import dataclass as plain_or_pydantic_dataclass
...
class Serializable:
    ...
    .. attention::
       A nested `Data` class be in autogenerated in two cases:

       - If the `Serializable` subclass is also a subclass of `pydantic.BaseModel`.
         + In this case, the class itself is used as the `Data` container.
         + An `encode` function is monkey-patched onto the class.
           (If it already exists, nothing is patched, but it is up to the developer
           to ensure that it follows the conventions of an `encode` function.)
       - If the `Serializable` subclass defines class annotations.
         + In this case it is assumed that
           * These annotations are all plain data attributes;
           * That those attributes are the ONLY ones needed to serialize the class.
         + Making these assumptions, a `Data` container will automatically be
           generated (wrapped with `pydantic.dataclass` if available, otherwise
       ...
       which just repeats what is already in the annotations.
    ...
    def __init_subclass__(cls):
        ...
        if Data_is_from_parent and cls.Data is not Dataclass.Data:  # Dataclass is a type we use for Data containers; it has special support to allow being used as a Data type
            ...
            if isinstance(cls, BaseModel):
                ...
                # NB: We must not subclass `cls`, because in some places we check for `isinstance(obj, cls.Data)`
                Data = cls
